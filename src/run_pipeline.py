    ...
    if deploy:
        ...
        try:
            ...
            if not role:
                ...
                print(
                    "In SageMaker environment, deployment would proceed automatically."
                )
            else:
                deployer = BatchDeployer(
                    role=role, bucket=bucket, prefix=prefix, region=region
                )
...
def main():
    ...
    parser.add_argument(
        "--prefix", type=str, default="sagemaker-housing", help="S3 prefix"
    )
    parser.add_argument("--region", type=str, default="us-east-1", help="AWS region")
    parser.add_argument(
        "--models",
        nargs="+",
        default=["xgboost", "knn", "sklearn-gbm"],
        help="Models to train",
    )
    parser.add_argument("--tune", action="store_true", help="Run hyperparameter tuning")
    parser.add_argument(
        "--deploy", action="store_true", help="Deploy for batch inference"
    )
    parser.add_argument(
        "--no-mock",
        action="store_true",
